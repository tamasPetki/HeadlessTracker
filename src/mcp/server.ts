...
const SERVER_INSTRUCTIONS = `headless-tracker is a local-first PORTFOLIO TRACKER for crypto holdings, prediction markets, and on-chain wallets (Bybit exchange + Binance exchange + MetaMask multi-chain + Polymarket + Solana wallets). All tools and apps in this server are scoped to that domain.
...
- "set up an account" / "add Bybit" / "add Binance" / "connect MetaMask" / "connect Solana" / "remove this connection" / "show my accounts" / "add a custom token" / "track another wallet" → render_settings (MCP App; live Settings panel with Accounts / Add Account / Wallets / Custom Tokens tabs)
